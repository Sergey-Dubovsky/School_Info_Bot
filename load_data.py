...
def save(session,data):
    ...
    for row in data:
        if row.student_fio and row.student_fio not in excl_list:  # Проверка не входит ли ученик в список бывших учеников 

            last_rec = session.query(Balance).filter_by(student_fio = row.student_fio, last = True).one_or_none()   
            ...
            if float(last_balance) != round(row.balance,2): # проверка изменился ли баланс
                session.query(Balance).filter_by(student_fio = row.student_fio, last = True).update({"last": False}, synchronize_session="fetch") #сброс призанка посленего баланса
                session.add(  #добавление записи с новым балансом 
                    Balance(student_fio=row.student_fio,last=True,
                        ...
                        excursion_delta=(round(row.excursion,2)-float(last_excursion)),
                        other=round(row.other,2),
                        other_delta=(round(row.other,2)-float(last_other)),
                            )
                        )        
                print(f"{row.student_fio} Баланс: {round(row.balance,2)} ({round(round(row.balance,2)-float(last_balance),2)})")
