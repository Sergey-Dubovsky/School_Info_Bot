...
@with_session
def save_last_balance(
    session, id, new_balance
):  # сохранение значения отправленного баланса для исключения повторной отправки одинакового баланса
    session.query(Student).filter_by(telegram_id=id).update(
        {"last_balance": new_balance}, synchronize_session="fetch"
    )
    session.commit()
...
def main():
    ...
    if debtors_list:
        ...
        for row in debtors_list:
            if (
                datetime.datetime.today().isoweekday() == 6
                or abs(row.last_balance - row.balance) > 200
            ):
                ...
                response = telegram.notify(
                    ...
                    message=message_text(row.student_fio, row.balance),
                )
