...
class Debtor(Base):
    ...
    last_balance = Column(Numeric(precision=10,scale=2))    # последний отправленный баланс (чтоб не слать повторное уведомление с таким же балансом)
    telegram_id = Column(String(10),nullable=False)         # chat_id пользователя из телеграмма     
